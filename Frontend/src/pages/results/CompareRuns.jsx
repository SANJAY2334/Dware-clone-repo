...
const CompareRuns = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:5000/api/comparisons/type/data");
        ...
        console.error("Error fetching data comparison runs:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="p-6 bg-gray-100 rounded-2xl min-h-screen">
      <h2 className="text-2xl bg-gray-200 p-4 rounded-2xl font-bold text-gray-900">Results – Data Runs</h2>
